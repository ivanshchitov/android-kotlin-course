...
Чтобы добавить линию привязки, необходимо кликнуть на кружок на элементте `TextView` и протянуть линию до края макета.
...
1. Удалить нижнюю линиию привязки.
...
3. Задать идентификатор для элемента **box_two_text**.
4. Задать серый цвет фона: `background`=**@android:color/darker_gray**.
5. Задать текст "Box Two", выделив его в отдельную строку в ресурсах.
