...
Поскольку свойство `nights` является `LiveData`, то при его обновлении и `nightsString` будет обновляться.
...
1. В класс `ViewModel` добавлены поля `Job` и `Scope` в контексте главного потока для создания и управления корутинами.
2. Добавлены поля `tonight` и `nights`, содержащие данные.
3. Добавлены методы для инициализации данных, использующие корутины в контексте ввода-вывода для получения данных из базы.
4. Добавлены методы обработчики кнопок "Start", "Stop" и "Clear", выполняющие вставку, обновление и удаление данных из базы. Методы запускают корутины на UI-потоке для обновления данных класса `ViewModel`, и используют IO-контекст для выполнения операций с базой данных.
...
Отображение данных из БД в виде строки — это примитивный пример. Такие данные необходимо отображать в специальных компонентах `RecyclerView`, которые будут рассмотрены в следующей лекции.
