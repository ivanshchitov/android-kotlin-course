...
Существует два виде интентов: явные (explicit) и неявные (implicit).  

### Explicit-интенты

Explicit-интенты исопльзуются для перехода к активности в рамках собственного приложения. Они требуют указания полного имени класса для выполнения действия
...
* `ACTION_VIEW` — Наиболее распространённое общее действие. Используется дл отображения какой-либо активности. Например, для открытия браузера.
