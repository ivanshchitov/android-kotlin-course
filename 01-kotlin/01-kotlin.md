...
Классы в Kotlin не могут иметь полей. Т.е. переменные, которые объявляются внутри класса только выглядят и ведут себя как поля из Java, хотя на самом деле являются свойствами, т.к. для них неявно реализуются методы `get` и `set`.
...
Для любителей NPE существует ещё один способ. Можно написать `b!!` и это вернёт либо **non-null** значение `b`, либо выкинет NPE:
